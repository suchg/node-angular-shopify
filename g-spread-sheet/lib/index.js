...
const getRows = async (options, range) => {
  validateOptions(options);
  options.sheet = options.sheet || '';
  options.retention = options.retention || 14;
  ...
  const { spreadsheetId, sheet } = options;
  const numberOfRowsToCheck = 100000;
  const columnsRange = range || `!A2:A${numberOfRowsToCheck + 1}`;
  const request = {
    spreadsheetId,
    range: `${sheet}${columnsRange}`,
  }
  return spreadsheets.valuesGet(sheets, request);
}
