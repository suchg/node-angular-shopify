...
const operations = {
  getSubscriptionRange: () =>{
    ...
    const frequency = 1; // consider a week
    const duration = 2; // consider 3 months
    let lastDate = moment().add( duration, 'm' );
    let arrDates = [];
    while ( currentDate < lastDate ) {
      currentDate.add(frequency, 'm');
  ...
  startSubscriptionPolling: () => {
    ...
    .then( (orders) => {
      orders.forEach( (order) => {
        ...
        const strInsertSubscription = `insert into subscription ( orderId, orderData, udpateDate, createdDate )
        values( ${Number(order.id)}, ${ dbcon.connection.escape(JSON.stringify(order))}, ${currentDateTime}, ${currentDateTime} )`;
        ...
        lineItems.forEach((lineItem) => {
          const productId = lineItem.product_id;
          if (productIds.indexOf(productId) != -1) {
            const arrDateRange = operations.getSubscriptionRange();
